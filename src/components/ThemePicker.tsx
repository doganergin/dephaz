...
import {
  ...
  applyPalette, clearPalette,
  PALETTE_KEY, LIGHT_PALETTE_KEY,
} from '@/lib/palettes';
  ...
  return (
    <button
      ...
      className={`relative rounded-xl p-2.5 border-2 transition-all hover:scale-[1.02] active:scale-[0.98] ${
        selected ? 'border-red-500 shadow-lg shadow-red-500/20' : 'border-transparent hover:border-gray-400/30'
      }`}
    >
      <div
...
export default function ThemePicker() {
  ...
  const [darkId, setDarkId] = useState<string>('zinc');
  const [lightId, setLightId] = useState<string>('l-default');
  const [darkMode, setDarkMode] = useState(false);

  useEffect(() => {
    // Dark palette
    const savedDark = localStorage.getItem(PALETTE_KEY);
    if (savedDark) {
      try { setDarkId(JSON.parse(savedDark).id); } catch { /* */ }
    }
    // Light palette
    const savedLight = localStorage.getItem(LIGHT_PALETTE_KEY);
    if (savedLight) {
      try { setLightId(JSON.parse(savedLight).id); } catch { /* */ }
    }

    const check = () => setDarkMode(isDark());
    check();
  ...
  const palettes = darkMode ? DARK_PALETTES : LIGHT_PALETTES;
  const selectedId = darkMode ? darkId : lightId;

  function select(palette: DarkPalette) {
    applyPalette(palette);
    if (darkMode) {
      setDarkId(palette.id);
      localStorage.setItem(PALETTE_KEY, JSON.stringify(palette));
    } else {
      setLightId(palette.id);
      localStorage.setItem(LIGHT_PALETTE_KEY, JSON.stringify(palette));
    }
  }
            ...
            <div>
              <p className="text-sm font-bold text-[var(--foreground)]">
                {darkMode
                  ...
                  : (TR ? 'Açık Tema Rengi' : 'Light Theme Color')}
              </p>
