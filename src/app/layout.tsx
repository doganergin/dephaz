      ...
      <head>
        ...
        <script
          dangerouslySetInnerHTML={{
            __html: `(function(){try{var t=localStorage.getItem('theme');var m=window.matchMedia('(prefers-color-scheme: dark)').matches;var d=false;if(t==='black'){document.documentElement.classList.add('black');d=true}else if(t==='dark'||(t==null&&m)){document.documentElement.classList.add('dark');d=true}var pk=d?'dh_dark_palette':'dh_light_palette';var p=localStorage.getItem(pk);if(p){try{var c=JSON.parse(p);var r=document.documentElement;r.style.setProperty('--background',c.background);r.style.setProperty('--card-bg',c.cardBg);r.style.setProperty('--border',c.border);r.style.setProperty('--muted',c.muted);r.style.setProperty('--foreground',c.foreground)}catch(e2){}}}catch(e){}})()`,
          }}
        />
